...
@section('content')
    <div class="container mx-auto px-4 sm:px-8">
        <div class="py-8">
            <div>
                <h2 class="text-2xl font-semibold leading-tight">Edit Article: {{ $article->title }}</h2>
            </div>
                        ...
                        <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
                            ...
                            <div>
                                ...
                                <select name="board_id" id="board_id" required
                                    ...
                                    @foreach($boards as $id => $name)
                                        <option value="{{ $id }}" {{ old('board_id', $article->board_id) == $id ? 'selected' : '' }}>
                                            {{ $name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <textarea name="content" id="content" rows="15" required
                                          class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('content', $article->content) }}</textarea>
                            ...
                            <div class="flex items-start">
                                <div class="flex items-center h-5">
                                    {{-- Handle checkbox value persistence correctly --}}
                                    <input id="is_notice" name="is_notice" type="checkbox" value="1" {{ old('is_notice', $article->is_notice) ? 'checked' : '' }}
